...
func walk(m mapstr.MapStr, expandPaths bool, wo walkObserver) error {
	return walkFullMap(m, m, path{}, expandPaths, wo)
}

func walkFull(o interface{}, root mapstr.MapStr, path path, expandPaths bool, wo walkObserver) (err error) {
...
func walkFullMap(m mapstr.MapStr, root mapstr.MapStr, p path, expandPaths bool, wo walkObserver) (err error) {
	for k, v := range m {
		var newPath path
		if !expandPaths {
