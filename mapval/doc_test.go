...
func Example() {
	...
	data := mapstr.MapStr{"foo": "bar", "baz": "bot", "count": 1}
...
func ExampleCompose() {
	// Composition is useful when you need to share mapper validation logic between validators.
...
func ExampleOptional() {
	dataNoError := mapstr.MapStr{"foo": "bar"}
	dataError := mapstr.MapStr{"foo": "bar", "error": true}
...
func ExampleIs() {
	...
	data := mapstr.MapStr{"foo": "bar", "count": 1}
...
func ExampleSlice() {
	...
	data := mapstr.MapStr{"foo": []string{"foo", "something"}}
