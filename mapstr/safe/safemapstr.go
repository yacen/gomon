...
// `.value`
func Put(data mapstr.MapStr, key string, value interface{}) error {
...
func mapFind(data mapstr.MapStr, key, alternativeKey string) (subMap mapstr.MapStr, subKey string) {
	// XXX This implementation mimics `mapFind`, both should be updated to have similar behavior
	...
	for {
		...
		v, ok := tryToMapStr(d)
		if !ok {
			v = mapstr.MapStr{alternativeKey: d}
			data[k] = v
		}
...
func tryToMapStr(v interface{}) (mapstr.MapStr, bool) {
	switch m := v.(type) {
	case mapstr.MapStr:
		return m, true
	case map[string]interface{}:
		return mapstr.MapStr(m), true
	default:
		return nil, false
	}
}
