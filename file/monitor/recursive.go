	...
	for {
		...
		case event, ok := <-watcher.inner.Events:
			...
			case fsnotify.Create:
				...
				err = watcher.tree.Visit(event.Name, file.PreOrder, func(path string, _ bool) error {
			...
			case fsnotify.Remove:
				err := watcher.tree.Visit(event.Name, file.PostOrder, func(path string, _ bool) error {
					watcher.deliver(fsnotify.Event{
						Name: path,
						Op:   event.Op,
					})
