...
// an event. The conversions can be described using an (optionally nested) mapstr.MapStr
...
type Mapper interface {
	...
	Map(key string, event mapstr.MapStr, data map[string]interface{}) multierror.Errors
...
// to the event on the key.
func (conv Conv) Map(key string, event mapstr.MapStr, data map[string]interface{}) multierror.Errors {
...
func (o Object) Map(key string, event mapstr.MapStr, data map[string]interface{}) multierror.Errors {
	subEvent := mapstr.MapStr{}
...
// event map.
func (s Schema) ApplyTo(event mapstr.MapStr, data map[string]interface{}, opts ...ApplyOption) (mapstr.MapStr, multierror.Errors) {
	if len(opts) == 0 {
...
func (s Schema) Apply(data map[string]interface{}, opts ...ApplyOption) (mapstr.MapStr, error) {
	event, errors := s.ApplyTo(mapstr.MapStr{}, data, opts...)
...
func applySchemaToEvent(event mapstr.MapStr, data map[string]interface{}, conversions map[string]Mapper) multierror.Errors {
