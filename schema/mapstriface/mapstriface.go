...
import (
	"encoding/json"
	"fmt"
	"github.com/yacen/gomon/datetime"
	"github.com/yacen/gomon/mapstr"
	"github.com/yacen/gomon/schema"
	"log"
	"time"
...
func (convMap ConvMap) Map(key string, event mapstr.MapStr, data map[string]interface{}) multierror.Errors {
	d, err := mapstr.MapStr(data).GetValue(convMap.Key)
	if err != nil {
...
func toStrFromNum(key string, data map[string]interface{}) (interface{}, error) {
	emptyIface, err := mapstr.MapStr(data).GetValue(key)
	if err != nil {
...
func toIfc(key string, data map[string]interface{}) (interface{}, error) {
	intf, err := mapstr.MapStr(data).GetValue(key)
	if err != nil {
...
func toTime(key string, data map[string]interface{}) (interface{}, error) {
	emptyIface, err := mapstr.MapStr(data).GetValue(key)
	if err != nil {
		return datetime.Time(time.Unix(0, 0)), schema.NewKeyNotFoundError(key)
	}
	...
	case time.Time:
		...
		if ok {
			return datetime.Time(ts), nil
		}
	case datetime.Time:
		ts, ok := emptyIface.(datetime.Time)
		if ok {
			return ts, nil
		}
	}
	...
	return datetime.Time(time.Unix(0, 0)), schema.NewWrongFormatError(key, msg)
}
