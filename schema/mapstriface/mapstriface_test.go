...
import (
	...
	s "github.com/yacen/gomon/schema"
)
...
func TestConversions(t *testing.T) {
	...
	expected := mapstr.MapStr{
		...
		"test_bool":                 true,
		"test_time":                 datetime.Time(ts),
		"common_time":               cTs,
		"test_obj_1": mapstr.MapStr{
			"test": "hello from top level",
		},
		"test_obj_2": mapstr.MapStr{
			"test": "hello, object",
		},
...
func TestFullFieldPathInErrors(t *testing.T) {
	...
	for _, c := range cases {
		...
		_, errs := c.Schema.ApplyTo(mapstr.MapStr{}, c.Input)
