...
type ApplyOption func(mapstr.MapStr, multierror.Errors) (mapstr.MapStr, multierror.Errors)

// AllRequired considers any missing field as an error, except if explicitly
// set as optional
func AllRequired(event mapstr.MapStr, errors multierror.Errors) (mapstr.MapStr, multierror.Errors) {
	k := 0
...
// as required
func FailOnRequired(event mapstr.MapStr, errors multierror.Errors) (mapstr.MapStr, multierror.Errors) {
	k := 0
	for i, err := range errors {
		if err, ok := err.(*KeyNotFoundError); ok {
			if !err.Required {
...
func NotFoundKeys(cb func(keys []string)) ApplyOption {
	return func(event mapstr.MapStr, errors multierror.Errors) (mapstr.MapStr, multierror.Errors) {
		var keys []string
