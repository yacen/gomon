...
func LoadCertificate(config *CertificateConfig) (*tls.Certificate, error) {
	...
	if err != nil {
		log.Fatalf("Failed reading certificate file %v: %+v", certificate, err)
		return nil, fmt.Errorf("%v %v", err, certificate)
	}

	keyPEM, err := ReadPEMFile(key, config.Passphrase)
	if err != nil {
		log.Fatalf("Failed reading key file %v: %+v", key, err)
		return nil, fmt.Errorf("%v %v", err, key)
	}

	cert, err := tls.X509KeyPair(certPEM, keyPEM)
	if err != nil {
		log.Fatalf("Failed loading client certificate %+v", err)
		return nil, err
	}

	log.Printf("loading certificate: %v and key %v", certificate, key)
	return &cert, nil
}
...
func ReadPEMFile(path, passphrase string) ([]byte, error) {
	...
	for len(content) > 0 {
		...
		if x509.IsEncryptedPEMBlock(block) {
			...
			if err != nil {
				log.Printf("Dropping encrypted pem '%v' block read from %v. %v",
					block.Type, path, err)
				continue
			}
...
func LoadCertificateAuthorities(CAs []string) (*x509.CertPool, []error) {
	...
	for _, path := range CAs {
		...
		if err != nil {
			log.Fatalf("Failed reading CA certificate: %v", err)
		...
		log.Printf("successfully loaded CA certificate: %v", path)
	}
